...
try(Hadamard(a, B), silent=TRUE)[[1]] # Hadamard function produces an error
```
...
# Making the Hadamard product
K1 <- A[IDrowA,IDcolA]*B
K2 <- Hadamard(A, B, IDrowA=IDrowA, IDcolA=IDcolA)
...
K2 <- Hadamard(A, B, IDrowB=IDrowB, IDcolB=IDcolB)
dim(K2) == dim(A) 
```
...
# Making the Hadamard product
K1 <- A[IDrowA,IDcolA]*B[IDrowB,IDcolB]
K2 <- Hadamard(A, B, IDrowA=IDrowA, IDrowB=IDrowB, IDcolA=IDcolA, IDcolB=IDcolB)
